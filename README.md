...
연세대학교 디지털전자회로(정성욱 교수님) 프로젝트 **〈Combinational circuit을 이용한 16-bit bus 설계〉** 보고서의 복원본입니다.

원본 보고서(Google Docs)가 유실되어, 수업 중 촬영한 사진 70장과 강의 전사 기록을 근거로 전체를 복원했습니다. 사진 70장을 전수 판독하고 핵심 수치 41건을 확대 재검증하여, 어디까지가 원본 판독이고 어디부터 재구성인지(`[복원]`, `(저신뢰)` 표기) 구분해 두었습니다.
...
> 복원 근거가 된 사진 원본(KakaoTalk_*.jpg/png)은 제3자가 포함되어 있어 저장소에는 포함하지 않았습니다 (로컬 보관).
